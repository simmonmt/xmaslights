...
TEST_F(CoordsTest, FinalCoords) {
  const std::string final_path = Path("final");
  QCHECK_OK(WriteFile(final_path, {"1 10,20,30", "2 40,50,60"}));

  absl::StatusOr<std::vector<CoordsRecord>> result =
      ReadCoords(std::nullopt, final_path);
  ASSERT_TRUE(result.ok());
